...
class LupFinalResponseEstimateCreate(BaseModel):
    model_config = ConfigDict(extra="forbid")

    model: LupModelIdentityCreate
    visible_character_count: int = Field(ge=0, le=20_000_000)
    visible_utf8_byte_count: int = Field(ge=0, le=80_000_000)
    visible_word_count: int = Field(ge=0, le=20_000_000)

    @model_validator(mode="after")
    def validate_visible_counts(self) -> "LupFinalResponseEstimateCreate":
        characters = self.visible_character_count
        utf8_bytes = self.visible_utf8_byte_count
        words = self.visible_word_count
        if characters == 0 and (utf8_bytes != 0 or words != 0):
            raise ValueError("zero visible characters require zero bytes and words")
        if utf8_bytes < characters:
            raise ValueError("UTF-8 byte count cannot be smaller than character count")
        if utf8_bytes > characters * 4:
            raise ValueError("UTF-8 byte count cannot exceed four bytes per character")
        if words > characters:
            raise ValueError("visible word count cannot exceed character count")
        return self
...
class LupFinalResponseCompleteCreate(BaseModel):
    ...
    usage: LupToolUsageCreate | None = None
    final_response_estimate: LupFinalResponseEstimateCreate | None = None
    ...
    def validate_completion_mode(self) -> "LupFinalResponseCompleteCreate":
        if self.completion_mode == "provider_exact":
            ...
            if self.final_response_estimate is not None:
                raise ValueError(
                    "provider_exact completion cannot include final_response_estimate"
                )
        elif self.completion_mode == "estimated_final_response":
            if self.usage is not None:
                raise ValueError(
                    "estimated_final_response usage is generated by the Gateway"
                )
            if self.final_response_estimate is None:
                raise ValueError(
                    "estimated_final_response requires numeric final_response_estimate"
                )
        else:
            if self.usage is not None:
            ...
            if self.final_response_estimate is not None:
                raise ValueError(
                    "final_response_estimate is only valid for estimated_final_response"
                )
...
class McpOAuthPresentationUpdate(McpStrictModel):
    profile_id: Literal["chatgpt-stable", "developer-dynamic", "agent-restricted"]
    presentation_mode: (
        Literal["catalog_broker", "deferred_native", "native_projected"] | None
    ) = None
