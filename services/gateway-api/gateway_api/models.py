...
class AgentToolCall(Base):
    ...
    traffic_task_usage_id: Mapped[str | None] = mapped_column(String(36), nullable=True, unique=True)
    traffic_correlation_id: Mapped[str | None] = mapped_column(String(36), nullable=True, unique=True)
    traffic_event_id: Mapped[str | None] = mapped_column(String(36), nullable=True, unique=True)
    traffic_observation_id: Mapped[str | None] = mapped_column(String(36), nullable=True, unique=True)
    traffic_delivery_status: Mapped[str] = mapped_column(String(32), default="not_recorded", index=True)
...
class OutboxEvent(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True)
    audit_event_id: Mapped[str] = mapped_column(ForeignKey("audit_events.id"))
    owner_subject: Mapped[str] = mapped_column(String(255), index=True)
    event_type: Mapped[str] = mapped_column(String(120), index=True)
    ...
    replayed_from_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, index=True)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
