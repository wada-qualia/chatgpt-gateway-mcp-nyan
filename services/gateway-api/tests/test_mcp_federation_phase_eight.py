...
def test_phase_eight_matrix_matches_runtime_policy() -> None:
    ...
    assert matrix["current_qualification_task"] == "CMG-FED-815"
    assert specification["current_protocol_version"] == MCP_CURRENT_PROTOCOL_VERSION
...
def test_protocol_admission_rejects_drafts_and_malformed_versions() -> None:
    ...
    for version in MODERN_MCP_PROTOCOL_VERSIONS:
        with pytest.raises(McpProtocolAdmissionError):
            negotiate_gateway_protocol_version(version)
        assert validate_http_protocol_version(version) == version
